...
class Block:
    def __init__(self, index, transactions, timestamp, previous_hash, nonce=0):
        ...
        :param index: Unique ID of each block
...
class Blockchain:
    ...
    def proof_of_work(block):
        """ Function that tries different value of nonce to get a hash that satisfies our difficulty
        criteria."""
        block.nonce = 0
    ...
    def check_chain_validity(cls, chain):
        ...
        for block in chain:
            ...
            #remove the hash field to recompute the has afain
...
def register_new_peers():
    node_address = requests.get_json()["node_address"]
    if not node_address:
...
def announce_new_block(block):
    for peer in peers:
        ...
        requests.post(url, data=json.dumps(block._dict_, sort_keys=True), headers=headers)